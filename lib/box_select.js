...
function parseOptions(button){var parsedOptions={clientId:button.getAttribute('data-client-id'),linkType:button.getAttribute('data-link-type'),multiselect:button.getAttribute('data-multiselect'),domain:button.getAttribute('data-domain')};var validationErrors=validateOptions(parsedOptions);if(validationErrors.length===0){configureOptions(parsedOptions);}
return validationErrors;}
...
var name='';var url=configuredOptions.domain+'/index.php?rm=box_select_view';url+='&client_id='+configuredOptions.clientId;url+='&link_type='+configuredOptions.linkType;url+='&multiselect='+configuredOptions.multiselect;var specs='height='+windowConfiguration.height+',width='+windowConfiguration.width;popup=window.open(url,name,specs);detectPopupClose();return true;}
function launchPopupViaProxy(){if(proxy){return false;}
proxy=document.createElement('iframe');var url=configuredOptions.domain+'/index.php?rm=box_select_proxy';url+='&client_id='+configuredOptions.clientId;url+='&link_type='+configuredOptions.linkType;url+='&multiselect='+configuredOptions.multiselect;proxy.src=url;proxy.height='0px';proxy.width='0px';document.body.appendChild(proxy);return true;}
